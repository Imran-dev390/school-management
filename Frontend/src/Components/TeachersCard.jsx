...
console.log("teachers",assignedClass);
  useEffect(() => {
...
setTimeout(() => setShowModal(false), 200); // Give the toast time to render
  ...
  {totalTeachers.map((teacher, idx) => (
    <div key={idx} className="bg-white shadow rounded-xl p-6 border">
      ...
      <div className="text-center mt-4">
        ...
        <p><strong>Class:</strong> {teacher.assignedClass?.name || "N/A"}</p>
        <p><strong>Section:</strong> {teacher.assignedClass?.section || "N/A"}</p>
