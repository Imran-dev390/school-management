...
//   // Example content layout (you can refine position later)
//   drawText(`Meritaleem Public School`, 200, 800, 16);
//   drawText(`Fee Voucher - Student Copy`, 400, 780, 12);

//   drawText(`Student Name: ${student.name}`, 50, 750);
//   drawText(`Father Name: ${student.parent}`, 300, 750);
//   drawText(`Class: ${student.Classs?.name || 'N/A'}`, 50, 730);
//   drawText(`GR No: ${student.grNo}`, 300, 730);
//   drawText(`Voucher No: VCH-${student._id.toString().slice(-5)}`, 50, 710);
//   drawText(`Voucher Month: ${dueDate.toLocaleString('default', { month: 'long', year: 'numeric' })}`, 300, 710);
//   drawText(`Issue Date: ${new Date().toLocaleDateString()}`, 50, 690);
//   drawText(`Due Date: ${dueDate.toDateString()}`, 300, 690);

//   drawText(`Monthly Fee: Rs. ${fee.amount}`, 50, 660);
//   drawText(`Concession: ${student.concession || 0}%`, 50, 640);
//   drawText(`Final Amount: Rs. ${finalAmount}`, 50, 620);

//   const finalAmountInWords = require("number-to-words")
//     .toWords(finalAmount)
//     .replace(/\b\w/g, c => c.toUpperCase());

//   drawText(`In Words: ${finalAmountInWords} Only`, 50, 600);

//   drawText(`Authorized Signature`, 400, 100);
...
function generateFeeVoucherPDF(student, fee, finalAmount, dueDate) {
  return new Promise((resolve, reject) => {
    const voucherId = `VCH-${student._id.toString().slice(-5)}-${Date.now().toString().slice(-4)}`;
    const className = student.Classs?.name || 'N/A';
    const fatherName = student.parent || 'N/A';
    const concession = student?.concession || 0;
    const feeType = fee.name || "N/A";
    const baseAmount = fee.amount || "N/A";
    const grNo = student.grNo || 'GR12345';
    const issueDate = new Date().toLocaleDateString();
    const finalAmountInWords = toWords(finalAmount).replace(/\b\w/g, char => char.toUpperCase());
    ...
    pdf.create(htmlContent, options).toBuffer((err, buffer) => {
      ...
      resolve(buffer); // Return the buffer directly
    });
  });
}
